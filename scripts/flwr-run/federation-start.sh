...
echo "FEDERATED LEARNING (Obstructive Apnea Predictions)" && echo ""

# Variables
rp="/home/admine/matej/federated-learning-poc" # repository path
lp="${rp}/src/federated/logs" # logs path
started_processes=""
echo "Logs can be found at ${lp}" && echo ""


# Virtual environment
echo "Setting up virtual environment" && echo ""
cd ${rp} # !important
source .venv/bin/activate

# Updates
echo "Setting up git (fetching updates)" && echo ""
...
### Superlink
echo "Setting up flower superlink" && echo ""
flower-superlink --insecure &> ${lp}/super_link.log &
started_processes="${!}"

### Supernodes
echo "Setting up flower supernodes" && echo ""
n=1
...
do
    ...
    echo "    Setting up hospital ${n}" && echo ""
    flower-supernode \
      --insecure \
...
echo "Setting up flower run (starting the learning process)" && echo ""
